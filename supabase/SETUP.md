...
5. No volver a ejecutar `202608030001_initial_portal_schema.sql` — esa migración ya corrió y no se modifica.
...
- toda escritura privilegiada (crear/editar organización, asignar vendedor, invitar, activar/desactivar, recuperación de contraseña) pasa exclusivamente por la Edge Function `admin-portal` — nunca por el navegador directo ni por `supabase.auth.admin`.
